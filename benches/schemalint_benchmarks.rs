...
// ---------------------------------------------------------------------------
// Benchmark: cold start (500 schemas, no cache)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Benchmark: incremental (500 schemas, with cache)
// ---------------------------------------------------------------------------
...
fn bench_incremental(c: &mut Criterion) {
    ...
    // Pre-populate cache with all but the last schema.
        ...
        b.iter(|| {
            for bytes in &schema_bytes {
                ...
                if let Some(cached) = cache.get(hash) {
                    let _diags = ruleset.check_all(&cached.arena, &profile);
                } else {
                    let value: serde_json::Value = serde_json::from_slice(bytes).unwrap();
                    let normalized = normalize(value).unwrap();
                    let _diags = ruleset.check_all(&normalized.arena, &profile);
