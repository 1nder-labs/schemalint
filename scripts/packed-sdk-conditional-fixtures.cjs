...
function conditionalFixtures({ field, model, OPENAI, AMBIGUOUS }) {
  const optionsSource = [
    ...
    '',
    '// Divergent branches are the point of this fixture: newer ai versions',
    '// infer the schema generic from the first branch, so the second no longer',
    '// unifies. Not @ts-expect-error - the minimum SDK still accepts this, and',
    '// an unused directive would fail that variant instead.',
    '// @ts-ignore',
