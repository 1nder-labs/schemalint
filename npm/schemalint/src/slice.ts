 ...
 */
export function buildSlicedModule(
  ...
  const enqueue = (names: Iterable<string>): void => {
  ...
  for (const stmt of sourceFile.statements) {
    ...
    if (kept.has(stmt)) parts.push(stmt.getText(sourceFile));
  }
...
function indexDeclarations(
  ...
  for (const stmt of sourceFile.statements) {
    for (const name of boundNames(stmt, tsModule)) {
      ...
      if (existing) existing.push(stmt);
      else map.set(name, [stmt]);
    }
  }
  return map;
}

function boundNames(stmt: ts.Statement, tsModule: typeof ts): string[] {
  if (tsModule.isVariableStatement(stmt)) {
    return stmt.declarationList.declarations.flatMap((decl) =>
      bindingNames(decl.name, tsModule)
    );
  }
  if (
...
function bindingNames(name: ts.BindingName, tsModule: typeof ts): string[] {
  if (tsModule.isIdentifier(name)) return [name.text];
  return name.elements.flatMap((el) =>
    tsModule.isOmittedExpression(el) ? [] : bindingNames(el.name, tsModule)
  );
}
 ...
 */
function identifierNames(node: ts.Node, tsModule: typeof ts): Set<string> {
  ...
  const visit = (n: ts.Node): void => {
    ...
    if (
      (tsModule.isPropertyAssignment(n) ||
...
function importIsReferenced(
  stmt: ts.ImportDeclaration,
  referenced: ReadonlySet<string>,
  tsModule: typeof ts
): boolean {
  const clause = stmt.importClause;
  if (!clause) return false; // side-effect-only import: no binding to reference
